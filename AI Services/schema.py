...
Struktur ini identik dengan skema yang dipakai `dataset_generator/generate_dataset.py`
saat membuat label sintetis. Kalau skema di sana berubah, ubah juga di sini —
generator sengaja tidak meng-import modul ini agar tetap bisa dijalankan tanpa
dependensi runtime inference.
"""
...
def parse_model_output(raw: str) -> FurnitureBOMData:
    ...
    supaya pemanggil bisa membedakan dua jenis kegagalan itu saat menghitung
    schema validity rate.
    """
    text = raw.strip()

    # Model kadang membungkus jawaban dalam ```json ... ```; buang pagarnya.
    if text.startswith("```"):
        text = text.split("\n", 1)[-1] if "\n" in text else text
