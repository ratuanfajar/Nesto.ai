...
class BOMConfig:
    ...
    drawer_front_height_mm: float = 150.0 # tinggi muka laci (di atas area pintu)
...
def _split_panel(
    ...
    """Pecah panel yang lebih besar dari satu lembar menjadi n keping sambungan.

    Mengembalikan [(panjang, lebar, qty)]. Tanpa ini, panel belakang lemari tinggi
    (mis. 1500 x 1900 mm) tidak muat di lembaran 2440 x 1220 dalam orientasi apa pun
    dan diam-diam hilang dari hasil nesting - kebutuhan triplek jadi kurang dihitung.
    """
...
def build_bom(data: FurnitureBOMData, cfg: Optional[BOMConfig] = None) -> List[Part]:
    ...
    gap = cfg.door_gap_mm
    front_h = carcass_h                                  # tinggi area muka
    drawer_zone = 0.0
...
def bom_summary(parts: Sequence[Part]) -> dict:
    """Ringkasan cepat: jumlah part, total keping, luas per tebal, panjang edging."""
    by_thickness: dict = {}
    for p in parts:
