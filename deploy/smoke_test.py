...
def make_client(url):
    ...
    os.environ["NESTO_MODEL_DIR"] = str(ROOT / "deploy" / "__no_model__")
...
def run_cpu_checks(c, spec, live):
    ...
    if not live:
        # Hanya bermakna di mode in-process: di mode live API key-nya milik user.
...
def run_gpu_checks(c, image_path):
    ...
    if check("/v1/extract 200", r.status_code == 200, r.text[:300]):
        d = r.json()
        # Dimensi bersarang di overall_dimensions, bukan di level teratas spec.
