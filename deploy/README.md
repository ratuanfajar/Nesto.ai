...
uv run python deploy/verify_model.py          # wajib: bobotnya fine-tuned atau bukan?
```
...
Setting dibaca dari `deploy/.env`, bukan dari env var di depan perintah:
...
membangun manual: `docker build -f deploy/Dockerfile -t nesto-ai:1.0.0 .` dari root.

### Tanpa Docker (pengembangan)

```bash
uv sync --extra serve
uv run python deploy/smoke_test.py        # endpoint CPU, model tidak dimuat
```
...
```bash
uv run python deploy/verify_model.py              # 2 sampel di CPU, ~30 detik/gambar
...
| 422 | `model_output_invalid` | model gagal membaca sketsa - minta foto ulang; `raw_output` disertakan untuk di-log |
