...
class ExtractResponse(BaseModel):
    ...
    raw_output: str              # teks asli model, berguna untuk debugging
    inference_seconds: float


class BOMResponse(BaseModel):
    parts: List[PartOut]
    summary: Dict               # part_types, total_pieces, area_m2_by_thickness, edging_length_m


class NestResponse(BaseModel):
    summary: Dict               # sheets_used, total_waste_pct, pieces_unplaced, ...
...
class AnalyzeResponse(BaseModel):
    ...
    raw_output: str
    timings: Dict[str, float]    # inference / bom / nesting / total, dalam detik
...
class ErrorDetail(BaseModel):
    error: str                   # kode stabil untuk dicabang frontend
    message: str                 # pesan bahasa Indonesia, boleh ditampilkan ke user
