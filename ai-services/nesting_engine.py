...
Membungkus `rectpack` (Guillotine / MaxRects): kerf ditambah sebelum pack lalu
dipotong lagi saat pelaporan; trim margin dibuang lebih dulu; `respect_grain`
mematikan rotasi (part `grain_locked=False` tetap boleh ditukar manual); satu
lembar tak mencampur material/tebal. Output: daftar `Sheet` + waste %.
"""
...
def nest_parts(parts: Sequence[Part], cfg: Optional[NestConfig] = None) -> NestResult:
    ...
    # Satu lembar tidak boleh campur bahan: kelompokkan per (material, tebal).
    ...
    for (material, thickness), group in sorted(groups.items(), key=lambda kv: -kv[0][1]):
        ...
        for bin_idx, x, y, w, h, r_id in packer.rect_list():
            ...
            real_l, real_w = w - kerf, h - kerf     # buang kerf: ukuran part sesungguhnya
