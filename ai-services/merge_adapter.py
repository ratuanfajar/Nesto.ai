...
Butuh ~10 GB RAM (CPU) dan menulis ~4.4 GB. Merge wajib di atas base non-quantized;
di atas bobot 4-bit hasilnya rusak. Semua config dibaca dari folder adapter.
"""
...
def resolve_out_dir(out: Path, force: bool) -> None:
    ...
    Dihapus (bukan ditimpa) karena shard sisa membuat index tidak konsisten dan
    model gagal di-load.
    """
    if not out.exists():
        return
...
def load_processor(adapter: Path, model_id: str):
    """Ambil processor dari folder adapter kalau ada (min/max pixels + chat template
    identik dengan training); kalau tidak, dari base model."""
