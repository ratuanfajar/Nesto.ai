"""Buktikan bobot yang akan disajikan deploy memang model hasil fine-tune.

Dijalankan lewat `app/vlm.py` (jalur kode yang sama dengan service): base model
mentah tak pernah menghasilkan JSON yang lolos `schema.py`, jadi parse sukses
dengan angka mendekati ground truth sudah cukup jadi bukti. Untuk kualitas di
banyak sampel, pakai `ai-services/evaluate.py`.

    uv run python be/verify_model.py                          # 2 sampel di CPU
